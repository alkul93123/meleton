<template id="">
  <el-row>
    <el-col>
      ...
      <el-row v-for="label, index in labels" :key="index" class="item--row" :class="{'is-collapsed': collapsed}">
        <el-col :span="6" class="item--label">
          <slot :name="`label_${item.alias}`" v-bind:item="item" >
            {{ label.name }}
          </slot>
        </el-col>
        <el-col :span="18" class="item--value">
          <slot :name="`value_${item.alias}`" v-bind:item="item" >
            {{ item[label.alias] }}
          </slot>
        </el-col>
      </el-row>
